...
`choiceTwo.calories = 150;` will cause an error because in the class `Snack`, `calories` is private, so it cannot be used in another class.
...
public class Book {
    ...
    public static void main(String[] args) {
        Book book1 = new Book();
        Book book2 = new Book("Harry Potter", "Fantasy", "J.K. Rowling", 965);
...
```
    ...
    REPL.$JShell$22C$Sport@73af4f38
...
public class BankAccount {
    ...
    public void withdraw(double amount) {
        if (amount > this.balance) {
            System.out.println("You do not have that enough money to withdraw " + this.balance + ". All the money was withdrawed instead.");
            this.balance = 0;
        } else {
            this.balance -= amount;
        }
...
```
    ...
    You do not have that enough money to withdraw 1300.0. All the money was withdrawed instead.
...
It can use both variables because it is a non static method.
...
Using `this()` in a constructor calls another constructor within the same class. This can be a constructor with no parameters, or one with parameters. When calling constructors with parameters, the correct arguments must be passed. Using `this()` helps reuse code between constructors and is especially beneficial when there are multiple constructors that can potentially make code hard to read.
